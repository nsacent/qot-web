    ...
    if (overlay) {
        return (
            <button
                ...
                aria-pressed={saved}
                className={`absolute right-3 top-3 z-20 flex h-9 w-9 items-center justify-center bg-transparent transition hover:scale-110 hover:text-orange-500 disabled:cursor-not-allowed disabled:opacity-60 ${saved ? "text-orange-500" : "text-white"}`}
            >
                <FontAwesomeIcon
                    icon={icon}
                    className={`h-5 w-5 ${checking || loading ? "animate-spin" : ""}`}
                />
            </button>
        );
    }
